...
def extract_price_details(records: List[Dict[str, Any]]) -> Dict[str, float]:
    ...
    for record in records:
        if record.get("STOK_KODU") is None and record.get("KDV_ORANI"):
            ...
            if kdv_key:
                price_details[kdv_key.lower().replace(" ", "_")] = record.get(
                    "TUTAR", 0
                )
...
def calculate_glass_item_price(
    ...
    glass_qty = item_doc.get("custom_glass_m2")
    ...
    for bom_item in bom.get("items"):
        ...
        record = next(
            (
                ...
                for r in records
                if r.get("STOK_KODU")
                and r.get("STOK_KODU").lstrip("#") == i_code
            ),
            None,
        )
        if record is None:
    ...
    for kit in kits:
        ...
        record = next(
            (
                ...
                for r in records
                if r.get("STOK_KODU")
                and r.get("STOK_KODU").lstrip("#") == kit_name
            ),
            None,
        )
        if record is None:
