...
def before_save(doc, method) -> None:
    ...
    if operation_name == "Profil Temin" or operation_name == "Sac Kesim":
        ...
        if doc.is_corrective_job_card:
            ...
            if is_new:
                handle_barcode_new_regular_job_card(doc, order_no, poz_no, operation_name)
            else:
                handle_barcode_update_regular_job_card(
                    doc, order_no, poz_no, operation_name
                )
...
def handle_barcode_new_regular_job_card(
    ...
    # print("USA:", unique_sanal_adet)
    # print("TSA:", target_sanal_adet)

    print("\n\n\n")
    # frappe.throw("Something")

    # print("QTY:", doc.for_quantity)
    # print("TD List Length:", len(tesdetay_list))
    ...
    doc.set("custom_barcodes", barcodes)
    print("\n\n\n-- New Regular Job Card -- (End)")


def handle_barcode_update_regular_job_card(
    doc, order_no: str, poz_no: str, operation_name: str
) -> None:
    print("\n\n\n-- Update Regular Job Card -- (Start)")

    barcodes = doc.custom_barcodes

    # job_card_ref == doc.name
    operation_data = frappe.get_all(
        "TesDetay Operation Status",
        filters={"job_card_ref": doc.name},
        fields=["parent", "status", "operation"],
    )

    # for o in operation_data:
    #     print(f"{o.parent}, {o.operation}, {o.status}")

    # for b in barcodes:
    #     print(f"{b.name, b.barcode, b.status}")

    print("\n\n\n-- Update Regular Job Card -- (End)")
    pass
